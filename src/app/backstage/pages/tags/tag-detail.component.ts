...
export class TagDetailComponent implements OnInit, OnChanges {
  ...
  get dialogTitle(): string {
    return this.mode === 'create' ? '新建标签' : '编辑标签'
  }
  ...
  onSubmit() {
    if (this.tagForm.valid) {
      ...
      if (this.mode === 'edit' && this.tag) {
        this.httpService.put<any>(`/api/admin/tags/${this.tag.id}`, formData).subscribe({
          next: (response) => {
            if (response.success) {
              ...
              this.messageService.add({
                severity: 'error',
                summary: '错误',
                detail: response.message || '更新标签失败'
              })
            }
          },
          error: (error) => {
            this.messageService.add({
              severity: 'error',
              summary: '错误',
              detail: error.error.message || '更新标签失败'
            })
          },
          complete: () => {
            this.submitting.set(false)
        ...
        this.httpService.post<any>('/api/admin/tags', formData).subscribe({
          next: (response) => {
            if (response.success) {
              this.messageService.add({
                severity: 'success',
              ...
              this.messageService.add({
                severity: 'error',
                summary: '错误',
                detail: response.message || '创建标签失败'
              })
            }
          },
          error: (error) => {
            this.messageService.add({
              severity: 'error',
              summary: '错误',
              detail: error.error.message || '创建标签失败'
            })
          },
          complete: () => {
            this.submitting.set(false)
