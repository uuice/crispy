...
import { formatSemanticSearchHit } from './formatResults'
import {
...
export const FRONTEND_ASSISTANT_TOOLS: FrontendAssistantToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      name: 'get_content',
      description: '按类型与 slug 获取单条公开内容详情（文章、页面、分类、标签）。',
      parameters: {
        type: 'object',
        properties: {
          type: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '按语义相似度搜索已发布文章与页面（需 PostgreSQL pgvector）。适合自然语言提问。',
      parameters: {
        type: 'object',
        properties: {
  ...
  switch (toolCall.name) {
    ...
    case 'semantic_search': {
      ...
      const collections = Array.isArray(args.collections)
        ? (args.collections.filter((c) => c === 'posts' || c === 'pages') as EmbeddableCollection[])
        : undefined
      ...
      result = rows.map(formatSemanticSearchHit)
      break
    }
