...
- [自建 Plugin 说明](#payload-plugins)
- [Admin AI 助手（对话）](#ai-agent)
- [前台 AI 助手（公开只读）](#frontend-ai-assistant)
- [二次开发指引](#extend)
...
统一入口 pnpm cli — 全部开发与运维命令；pnpm cli help 查看分组，pnpm cli help <命令> 查看单条备注。
...
配置中心（Catalog + Active + Override）详见 #config-center。

### Posts 内嵌 Block（Lexical BlocksFeature）
...
### Collection 级 Access（按 permission）
...
Payload @payloadcms/plugin-mcp 提供 JSON-RPC 端点，供 Cursor / Claude 等 Agent 读写 CMS。

```
端点: POST http://localhost:3333/api/mcp
...
与后台 AI 助手（src/ai/agent/resources.ts）范围对齐；差异时以 plugins/index.ts mcpPlugin 为准。

### Globals（与 Agent 对齐）

mcpPlugin.globals 启用 header、footer、site-settings、cache-settings、comment-settings、ai-settings。自动生成 findHeader / updateHeader、findSiteSettings / updateSiteSettings 等；各 Global update 受 Payload access + Permission（settings:*）限制。

### 自定义 Tools（缓存 / 恢复 / 语义搜索）

除 Collection / Global 自动生成工具外，mcpPlugin.mcp.tools 注册与 Admin AI 助手对齐的自定义工具（src/plugins/mcpCustomTools.ts）。需在 API Key 的 payload-mcp-tool 组中单独开启；缓存相关需 cache:manage。
...
配置后在 Cursor MCP 或 curl 调 POST /api/mcp 即可验证连通。

<h2 id="cms-operations">运营能力（重定向 / 邮件 / 导入导出 / 搜索）</h2>

### URL 重定向（Redirects 插件 + middleware）

Admin → 运营 → 重定向 配置 from → to（自定义 URL 或关联 pages/posts）。修改后约 60 秒内前台生效，无需重新构建。
...
payload.config.ts 通过 src/email/createEmailAdapter.ts 注册邮件适配器。只读 Admin「邮件设置」写入的 .data/email-runtime.json（无 .env 回退）。无通道时表单仍可提交并入库，但不会发信。
...
1. pnpm cli db:migrate && pnpm cli db:status
2. pnpm cli dev:build && pnpm cli dev:start（或 Docker standalone，端口 3333）
3. Admin 配置中心：LLM 提供商、存储目标、集成凭证、邮件通道（见 #config-center）
...
Crispy 是 Payload 3 上的二次开发，不是 fork。与官方同步的核心原则：只升 npm 依赖、不改 node_modules；所有 @payloadcms/* 与 payload 锁同一版本；生产 Schema 只走迁移（DATABASE_PUSH=false），Admin 定制走 generate:importmap。

### 职责边界（升级时哪里会动）

| 层 | 随 Payload 升级可能变 | Crispy 自有（相对稳定） |
| --- | --- | --- |
...
1. 阅读 Payload Release Notes / Breaking Changes（重点：Plugin API、Admin 组件、DB adapter）
2. 开独立分支 upgrade/payload-x.y.z，不与功能开发混发
3. 一次性升级全部 @payloadcms/* 与 payload 到目标版本 → pnpm install
4. pnpm cli generate:types && pnpm cli generate:importmap（Admin Custom 组件必跑）
5. 若 Collection/Global/Block 字段有变：pnpm cli db:create <name> → 审阅 src/migrations/ → pnpm cli db:migrate（需 Postgres）
6. pnpm cli quality:ci
7. 手工冒烟（见下方 Checklist）
8. 生产：先 db:migrate，再发应用包（Linux 包不含 .env，服务器 cp .env.example .env）

```
# 示例：升级到 3.86.0（版本号按 release 为准，以下包须全部同版本）
...
| 区域 | 风险 | 升级后重点测 |
| --- | --- | --- |
| Admin importMap | Custom 组件路径/ props 变更 | cache、stats、ai-agent 页面能打开 |
...
1. Admin：登录、Collection 列表/编辑/保存、回收站切换
...
9. 部署包：pnpm cli dev:pack-linux 能构建；服务器 ./start.sh 或 ./pm2.sh start
...
Crispy 前台缓存仅持久化页面 HTML（Collection frontend-cache-entries，kind=route）。Middleware 在 HIT 时从 DB 直出 HTML；RSC 渲染时直接查 Payload，不再单独缓存 JSON 数据。TTL 由 cache-settings.pageRevalidateSeconds 控制。
...
前台各 page.tsx 须在本文件内写 export const revalidate = false（Next 不支持 re-export）。RSS 等 Route Handler 的 Cache-Control s-maxage 运行时读取 getResolvedCacheSettings().pageRevalidateSeconds。
...
getResolvedCacheSettings() 直接 findGlobal("cache-settings")，带 60s 进程内缓存，且不经过 DB 缓存层（避免循环依赖）。constants.ts 中 DEFAULT_PAGE_REVALIDATE 仅作为 Global 字段默认值，与运行时 TTL 对齐。

### HIT / MISS / STALE / BYPASS 判定
...
bypass 条件（middlewareCache.ts）：URL 带 ?nocache、存在 __prerender_bypass 或 __next_preview_data Cookie（草稿预览）。
...
```
# 验证（需 exposeCacheHeaders=true 且 cachingEnabled=true）
...
缓存清除仅通过 /admin/cache 手动执行（按路径、清除全部、清理过期）。内容变更不会自动删除 HTML 缓存。
...
注册表由 discoverFrontendRoutes 自动扫描 app/(frontend) 的 page.tsx / route.ts 路径，供 Admin /admin/cache 按 path 手动清除。
...
Payload 的 delete() 为硬删；启用 trash 后须 update({ deletedAt }) 才能移入回收站。Crispy 统一通过 src/utilities/trashOrDeleteDocument.ts 处理，AI Agent 的 delete_document 工具已接入。
...
完整步骤、高风险区域与 Checklist 见独立章节 #payload-upgrade。此处仅保留原则：@payloadcms/* 全家桶同版本；大版本单独分支；改 Admin 组件后必跑 generate:importmap。
...
<h2 id="payload-plugins">自建 Plugin 说明</h2>
...
1. 新建 src/plugins/enableXxx.ts，map collections 或改 config
2. 在 src/plugins/index.ts 注册
3. 若改 Admin 组件：pnpm cli generate:importmap
4. 若改 schema：pnpm cli db:create <name> 并 commit 迁移
...
与前台助手相同：POST 返回 text/event-stream，客户端用 fetch + consumeAgentStream（src/components/AdminAiAgent/consumeAgentStream.ts）解析。区别是请求体可带 sessionId，响应流中额外推送 { type: "session", sessionId } 用于会话持久化。完整协议说明见 #frontend-ai-assistant 章节「请求与流式传输」。
...
前台 AI 助手使用两条 HTTP 请求，对话走 SSE（Server-Sent Events）流式响应，不是 WebSocket。客户端用 fetch POST + response.body.getReader() 手动解析 SSE，而非浏览器原生 EventSource——因为 EventSource 仅支持 GET，无法 POST 携带 messages 历史。
...
1. 上游 LLM：openAiChatCompletionWithToolsStream 请求 OpenAI 兼容 /v1/chat/completions，body 设 stream: true；提供商按 token 推送 SSE，服务端逐块解析。
2. Crispy API：route.ts 创建 ReadableStream，runFrontendAssistantStream 每 yield 一个 AgentStreamEvent 即 enqueue 一行 data: {...}\n\n，连接保持不关闭直至 done/error。
3. 浏览器：useFrontendAiAssistant → fetch POST 后调用 consumeAgentStream；response.body.getReader() 循环 read()，按 \n\n 切分并 JSON.parse，onText 回调更新 React state，界面逐字刷新。

与普通 POST JSON 的区别：JSON 需等模型与工具全部执行完才返回完整 body；SSE 在 LLM 生成每个 token、工具开始/结束时立即推送，用户可见「打字机效果」与检索状态，无需长时间白屏等待。
